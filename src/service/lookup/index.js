...
async function lookup (words, save) {
  ...
  if (
    ...
    // 保存生词本
    if (save) {
      saveInfo = await wordbookService.save(localData.id)
    }
  ...
  if (localData && localData.id) {
    ...
    await historyDao.update(localData.id, data)
  } else {
    debug('No local cache hits')
    let historyData = await historyDao.create(data)
    historyId = historyData.id
  }

  // 保存生词本
  if (save) {
    saveInfo = await wordbookService.save(historyId)
  }
